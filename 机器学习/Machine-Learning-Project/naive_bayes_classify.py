...
plot_learning_curve(model, "Naive Bayes (all features)", x_train , y_train , ax=plt.gca(), ylim = [0.7, 1.05],n_jobs=20, cv=3)
plt.show()
classify(model,x_train,y_train,x_test,y_test)

model=naive_bayes.GaussianNB()
plot_learning_curve(model, "Naive Bayes (four features)", x_train1 , y_train1 , ax=plt.gca(), ylim = [0.7, 1.05],n_jobs=20, cv=3)
plt.show()
